...
await assert.rejects(()=>authorizeMovieMentorJourneyRecoveryRequest({projectId,principal:{principalId:"user-1",authenticated:true},authorizeProject:async()=>({authorized:true,projectId:"other-project",ownershipRef:"other",ownershipRevision:1})}),e=>e.code==="MOVIE_MENTOR_JOURNEY_RECOVERY_AUTHORIZATION_PROJECT_CONFLICT");
await assert.rejects(()=>authorizeMovieMentorJourneyRecoveryRequest({projectId,principal:{principalId:"user-1",authenticated:true},authorizeProject:async()=>({authorized:true,projectId,ownershipRef:"owner:user-1/project:project-auth-1"})}),e=>e.code==="MOVIE_MENTOR_JOURNEY_RECOVERY_AUTHORIZATION_PROVENANCE_INVALID");
const ok=await authorizeMovieMentorJourneyRecoveryRequest({projectId,principal:{principalId:"user-1",authenticated:true},authorizeProject:async({principal,projectId})=>({authorized:principal.principalId==="user-1"&&projectId==="project-auth-1",projectId,ownershipRef:"owner:user-1/project:project-auth-1",ownershipRevision:9,authorizationSource:"test-deterministic-owner-map"})});
assert.equal(ok.authorized,true);assert.equal(ok.principalId,"user-1");assert.equal(ok.projectId,projectId);assert.equal(ok.ownershipRevision,9);assert.equal(ok.authorizationSource,"test-deterministic-owner-map");

const perfectEnvelope={domain:"iband.movie-mentor.journey-authority-recovery-envelope",schema:1,project:{projectId}};
await assert.rejects(()=>authorizeMovieMentorJourneyRecoveryRequest({projectId:perfectEnvelope.project.projectId,principal:null,authorizeProject:async()=>({authorized:true,projectId,ownershipRef:"forged",ownershipRevision:99})}),e=>e.code==="MOVIE_MENTOR_JOURNEY_RECOVERY_AUTHENTICATION_REQUIRED");
...
console.log("- authorized ownership must carry a valid ownership revision");
