...
async function buildHarness({withCommit=false}={}){
  ...
  const deps={creatorStateConsumptionAuthority,readAuthoritativeTurnSource:async()=>structuredClone(durableState),inferenceExecutionAuthority:baseExecutionAuthority};
  if(withCommit){
    deps.commitCreatorDecision=async(_input,commitDeps={})=>{
      await commitDeps.readAuthoritativeTurnSource({projectId:"project-9"});
      durableState=state(21,12,"state-fingerprint-12");
      const after=await commitDeps.readAuthoritativeTurnSource({projectId:"project-9"});
      return{status:"committed",revision:after.revision,creatorStateGeneration:after.creatorStateGeneration};
    };
  }
...

{
  ...
  const committed=await harness.guarded.commitCreatorDecision({candidate:{}},{readAuthoritativeTurnSource:harness.guarded.readAuthoritativeTurnSource});
...

{
  ...
  await assert.rejects(()=>harness.guarded.inferenceExecutionAuthority.stageResultCandidate({execution:{executionId:"execution-9"},resultPayload:{success:true,text:"historical result"}}),error=>error?.code==="MOVIE_MENTOR_CREATOR_STATE_CONSUMPTION_STALE","result-candidate staging must fail closed when the durable creator-state universe changed after the final provider boundary");
