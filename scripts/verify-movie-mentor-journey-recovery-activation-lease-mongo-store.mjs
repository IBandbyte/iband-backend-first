...
const takeover = await store.replaceLease(lease(2, "process-B", "deploy-B", t1, t2, "B"), { expectedLeaseGeneration: 1, expectedLeaseReference: first.leaseReference, requireDurablyExpired: true }); assert.equal(takeover.leaseGeneration, 2); assert.notEqual(takeover.fencingToken, first.fencingToken);
