...
for(const identity of [
  ...
  assert.match(
    storeSource,
    identity,
    "canonical identity must remain schema-declared unique"
  );
}
...
assert.equal(
  status.physicalUniqueIndexReadiness,
  true
);

assert.equal(
  status.uniquenessReadinessRequired,
  true
);

assert.equal(
  status.readinessBoundary,
  "before-canonical-read-or-finalization-transaction"
);
...
assert.equal(
  indexReads,
  1,
  "successful physical readiness must be cached for the store instance"
);
...
await assert.rejects(
  ()=>blockedStore.commit(
    {executionId:"exec_1"},
    {expectedProviderEffectRealityRevision:0}
  ),
  error=>
    error?.code===
...
assert.equal(
  transactionStarts,
  0,
  "missing physical uniqueness must fail closed before a Mongo session or finalization transaction begins"
);
...
const blockedReadStore=createMovieMentorCanonicalResultMongoStore({
  mongoModel:{
    findOne(){
      underlyingReads++;
      throw new Error("underlying read must not run");
    }
  },
  connect:async()=>{},
  readIndexes:async()=>missingReservation,
...
assert.equal(
  underlyingReads,
  0,
  "missing physical uniqueness must fail closed before canonical read delegation"
);
...
assert.ok(
  readyMatch,
  "verifier must locate formatted readyForCommit"
);
...
const canonicalPhysicalAt=
  storeSource.indexOf(
    "await physicalUniqueIndexReadiness();",
    storeSource.indexOf("async function readyForCommit")
  );

const crossLedgerPhysicalAt=
  storeSource.indexOf(
    "await crossLedgerPhysicalUniqueIndexReadiness();",
    storeSource.indexOf("async function readyForCommit")
  );
...
const transactionAt=
  storeSource.indexOf("session.withTransaction");

assert.ok(
  canonicalPhysicalAt>=0&&
  crossLedgerPhysicalAt>=0&&
  canonicalPhysicalAt<crossLedgerPhysicalAt&&
  crossLedgerPhysicalAt<transactionAt,
  "canonical and cross-ledger physical authority must both precede irreversible transaction authority"
);

console.log(
  "PASS canonical finalization owns actual five-index physical uniqueness proof and fails closed before read/finalization delegation"
);

console.log(
  "PASS canonical finalization independently crosses execution and result-candidate physical uniqueness before irreversible transaction authority"
);

console.log(
  "LAW: CANONICAL FINALIZATION MAY NOT BORROW EXECUTION OR RESULT-CANDIDATE PHYSICAL IDENTITY FROM THE AUTHORITY NEXT DOOR."
);