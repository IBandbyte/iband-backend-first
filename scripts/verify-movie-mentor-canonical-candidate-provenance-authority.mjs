...
const match=source.match(/function candidateBindingMatches\(candidate,result,execution\)\{([^}]*)\}/);
assert.ok(match,"canonical finalization must own an execution-aware candidate-binding court");
const court=match[1];
for(const field of ["stagedFromLeaseGeneration","stagedFromLeaseReference","stagedFromFencingToken","creatorStateRevision","creatorStateGeneration","creatorStateFingerprint","creatorStateOwnershipRef","creatorStateOwnershipRevision","stagedAt"]){assert.ok(court.includes(field),`canonical finalization must reject a schema-2 candidate stripped of ${field}`);}
...
assert.match(source,/candidateBindingMatches\(candidate,next,execution\)/,"the irreversible finalization call site must supply the exact durable execution to the candidate provenance court");
