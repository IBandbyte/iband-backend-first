...
const report={generatedAt:new Date().toISOString(),baseUrl:BASE_URL,passed:false,provider:null,model:null,liveAgents:[],executionStatus:null,contributions:[],skipped:[],failures:[],error:null};
...
async function run(){
  ...
  report.executionStatus=body?.status||null;
  report.skipped=Array.isArray(body?.skipped)?body.skipped:[];
  report.failures=Array.isArray(body?.failures)?body.failures:[];
  report.contributions=Array.isArray(body?.contributions)?body.contributions.map(c=>({agentId:c.agentId,observationCount:c.observations?.length||0,suggestionCount:c.provisionalSuggestions?.length||0,riskCount:c.risksAndConflicts?.length||0,confidence:c.confidence,authority:c.authority,mayAdvanceJourney:c.mayAdvanceJourney,creatorFacing:c.creatorFacing,mayOverwriteCreatorTruth:c.mayOverwriteCreatorTruth,requiresMentorSynthesis:c.requiresMentorSynthesis,creatorConfirmedDependencies:c.creatorConfirmedDependencies||[]})):[];
