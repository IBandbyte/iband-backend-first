...
const readPhysicalIndexes=async collection=>{
  if(collection==="movie_mentor_canonical_result"){
    return physicalIndexes;
  }

  if(Object.hasOwn(crossLedgerPhysicalIndexes,collection)){
    return crossLedgerPhysicalIndexes[collection];
  }
...
const resultDigest=hash(payload);

const record={
  resultReference:"canonical-result-1",
...
const legacyModel={
  ...
  async create(){
    legacyCreates++;
    throw new Error("legacy writer must never be reached");
  }
};

const legacyStore=createMovieMentorCanonicalResultMongoStore({
  mongoModel:legacyModel,
...
await assert.rejects(
  ...
  e=>e.code==="MOVIE_MENTOR_CANONICAL_RESULT_STORE_AUTHORITY_REQUIRED"
);

assert.equal(legacyCreates,0);

const closureCurrent=async()=>({
  authorized:true,
  closed:true,
...
const legacyAuthority=createMovieMentorCanonicalResultAuthority({
  store:legacyStore,
  assertCurrentClosure:closureCurrent,
  readResultCandidate:candidateReader,
  now:()=>new Date(record.committedAt),
  randomId:()=>"1"
});

await assert.rejects(
  ()=>legacyAuthority.commitResult({
    closure:{
      authorized:true,
      closed:true,
      executionId:"exec-1",
...
let candidateRow={
  ...
  resultDigest,
  resultPayload:stable(payload),
  stagedFromLeaseGeneration:3,
  stagedFromLeaseReference:"lease-1",
  stagedFromFencingToken:"fence-1",
  creatorStateRevision:9,
  creatorStateGeneration:4,
  creatorStateFingerprint:"state-fingerprint-1",
  creatorStateOwnershipRef:"ownership-1",
  creatorStateOwnershipRevision:2,
  stagedAt:"2031-12-31T23:59:59.000Z"
};

const executionCollection={
  async findOne(){
    return structuredClone(executionRow);
  },
  async updateOne(filter,update){
    assert.equal(filter.executionId,"exec-1");
    ...
    executionRow={
      ...
      resultFinalizationBarrierRevision:
        executionRow.resultFinalizationBarrierRevision+1
    };
...
const candidateCollection={
  async findOne(){
    return structuredClone(candidateRow);
  }
};
...
const authoritativeStore=createMovieMentorCanonicalResultMongoStore({
  mongoModel:authoritativeModel,
  executionCollection,
  candidateCollection,
...
const committed=await authoritativeStore.commit(
  record,
  {expectedProviderEffectRealityRevision:7}
);
...
const replay=await authoritativeStore.commit(
  record,
  {expectedProviderEffectRealityRevision:7}
);
...
await assert.rejects(
  ()=>authoritativeStore.commit(
    record,
    {expectedProviderEffectRealityRevision:6}
  ),
  e=>e.code==="MOVIE_MENTOR_CANONICAL_RESULT_REALITY_STALE"
);

const originalCandidate=candidateRow;

candidateRow={
  ...candidateRow,
  resultDigest:hash({tampered:true}),
  resultPayload:{tampered:true}
};
...
await assert.rejects(
  ...
  e=>e.code==="MOVIE_MENTOR_CANONICAL_RESULT_CANDIDATE_LINEAGE_STALE"
);
...
console.log("✓ exact CLOSED execution, current proof-bearing durable candidate lineage, provider-effect revision, physical unique-index reality, and atomic FINALIZED transition remain store-owned proofs");
console.log("✓ canonical finalization independently observes canonical-result, execution, and result-candidate physical uniqueness before acquiring cross-ledger transaction authority");
console.log("LAW: NO COMPONENT GETS CREDIT FOR ITS NEIGHBOUR'S PROOF. NO STORE-OWNED EXECUTION + CURRENT CANDIDATE LEDGER + PHYSICAL UNIQUENESS PROOF -> NO CANONICAL RESULT WRITE AUTHORITY.");