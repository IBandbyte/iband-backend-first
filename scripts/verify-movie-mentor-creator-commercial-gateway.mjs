...
const authProof=Object.freeze({ready:true,domain:"iband.movie-mentor.production-authentication-composition",provider:"clerk",verifyCredential:async()=>({verified:true}),expectedIssuer:"issuer",expectedAudience:"audience",verifierVersion:"1.0.0",verifierDomain:"iband.movie-mentor.journey-recovery-clerk-credential-verifier"});
...
assert.throws(()=>createMovieMentorProductionCreatorCommercialComposition({authentication:{ready:true,verifyCredential:authProof.verifyCredential,expectedIssuer:"issuer",expectedAudience:"audience"},purchaseIntentAuthority:provenPurchase,checkoutAuthority:provenCheckout,packageCatalogueAuthority:provenCatalogue}),e=>e.code==="MOVIE_MENTOR_CREATOR_COMMERCIAL_AUTHENTICATION_REQUIRED");
assert.throws(()=>createMovieMentorProductionCreatorCommercialComposition({authentication:authProof,purchaseIntentAuthority,checkoutAuthority:provenCheckout,packageCatalogueAuthority:provenCatalogue}),e=>e.code==="MOVIE_MENTOR_CREATOR_COMMERCIAL_PURCHASE_INTENT_REQUIRED");
assert.throws(()=>createMovieMentorProductionCreatorCommercialComposition({authentication:authProof,purchaseIntentAuthority:provenPurchase,checkoutAuthority,packageCatalogueAuthority:provenCatalogue}),e=>e.code==="MOVIE_MENTOR_CREATOR_COMMERCIAL_CHECKOUT_REQUIRED");
assert.throws(()=>createMovieMentorProductionCreatorCommercialComposition({authentication:authProof,purchaseIntentAuthority:provenPurchase,checkoutAuthority:provenCheckout,packageCatalogueAuthority:{listCommercialPackages}}),e=>e.code==="MOVIE_MENTOR_CREATOR_COMMERCIAL_PACKAGE_CATALOGUE_REQUIRED");
assert.throws(()=>createMovieMentorProductionCreatorCommercialComposition({authentication:authProof,purchaseIntentAuthority:{...provenPurchase,getStatus:()=>({...purchaseProof,durablePurchaseIntent:false})},checkoutAuthority:provenCheckout,packageCatalogueAuthority:provenCatalogue}),e=>e.code==="MOVIE_MENTOR_CREATOR_COMMERCIAL_PURCHASE_INTENT_REQUIRED");
assert.throws(()=>createMovieMentorProductionCreatorCommercialComposition({authentication:authProof,purchaseIntentAuthority:provenPurchase,checkoutAuthority:{...provenCheckout,getStatus(){throw new Error("status unavailable");}},packageCatalogueAuthority:provenCatalogue}),e=>e.code==="MOVIE_MENTOR_CREATOR_COMMERCIAL_CHECKOUT_REQUIRED");
assert.throws(()=>createMovieMentorProductionCreatorCommercialComposition({authentication:authProof,purchaseIntentAuthority:provenPurchase,checkoutAuthority:provenCheckout,packageCatalogueAuthority:{...provenCatalogue,getStatus:()=>({...catalogueProof,creatorMutable:true})}}),e=>e.code==="MOVIE_MENTOR_CREATOR_COMMERCIAL_PACKAGE_CATALOGUE_REQUIRED");
const production=createMovieMentorProductionCreatorCommercialComposition({authentication:authProof,purchaseIntentAuthority:provenPurchase,checkoutAuthority:provenCheckout,packageCatalogueAuthority:provenCatalogue});assert.equal(production.ready,true);assert.equal(production.mounted,false);assert.equal(production.purchaseIntentStatus.durablePurchaseIntent,true);assert.equal(production.checkoutStatus.durableCheckoutBinding,true);assert.equal(production.catalogueStatus.serverOwned,true);
...
const creatorSource=fs.readFileSync(new URL("../ai/MovieMentorProductionCreatorCommercialComposition.js",import.meta.url),"utf8");assert.match(creatorSource,/authenticationProven\(authentication\)/);assert.match(creatorSource,/ownedStatus\(purchaseIntentAuthority\)/);assert.match(creatorSource,/ownedStatus\(checkoutAuthority\)/);assert.match(creatorSource,/ownedStatus\(packageCatalogueAuthority\)/);assert.doesNotMatch(creatorSource,/typeof listCommercialPackages!=="function"/);
...
console.log("✓ creator gateway consumes production authentication identity rather than ready+method shape");
