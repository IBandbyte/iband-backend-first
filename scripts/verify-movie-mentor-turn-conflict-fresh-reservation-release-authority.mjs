...
const durable = {
  ...
  responseBlueprint: null, communicationPlan: null,
};

let findCalls = 0, reserveCalls = 0, openExecutionCalls = 0, releaseUnboundCalls = 0;
let readReservationCalls = 0, acquireCalls = 0, providerWorkCalls = 0, orchestrationCalls = 0;
let freshReleased = false;

const winningExecution = {
  found: true, authorized: true, executionId: "execution-winner-existing", creatorTurnId,
  principalId, projectId, reservationId: winningReservationId, requestDigest: "winner-request-digest",
  phase: "active", ownerId: "winner-owner", leaseGeneration: 1, leaseReference: "winner-lease",
  fencingToken: "winner-fence", leaseExpiresAt: "2099-01-01T00:00:00.000Z",
};
...
const inferenceSpendAuthority = {
  reserveTurn: async ({ serverAuthority, projectId: requestedProjectId, creatorTurnId: requestedCreatorTurnId }) => {
    ...
    return { authorized: true, reservationId: freshReservationId, principalId, projectId, operation: "movie-mentor-turn", units: 1, status: "reserved" };
  },
  readReservation: async ({ reservationId, principalId: requestedPrincipalId, projectId: requestedProjectId }) => {
    ...
    assert.equal(requestedProjectId, projectId);
    return { authorized: false, rehydrated: true, reservationId: winningReservationId, principalId, projectId, operation: "movie-mentor-turn", units: 1, status: "released" };
  },
};

const forbiddenProvider = async () => { providerWorkCalls += 1; throw new Error("invalid winning reservation must stop before provider work"); };

const inferenceExecutionAuthority = {
  findExecutionByCreatorTurn: async () => {
    findCalls += 1;
    return findCalls <= 2 ? { found: false, authorized: false } : structuredClone(winningExecution);
  },
  openExecution: async () => {
  ...
  acquireExecution: async () => { acquireCalls += 1; throw new Error("INVALID_RESERVATION_CROSSED_ACQUIRE_BOUNDARY"); },
  assertFence: forbiddenProvider, claimProviderCall: forbiddenProvider, beginProviderDispatch: forbiddenProvider,
  assertProviderDispatch: forbiddenProvider, contributeProviderEffectEvidence: forbiddenProvider,
  beginExecutionClosing: forbiddenProvider, reconcileExecutionClosure: forbiddenProvider,
  assertCurrentExecutionClosure: forbiddenProvider, stageResultCandidate: forbiddenProvider,
  readResultCandidate: async () => null, commitCanonicalResult: forbiddenProvider,
...
const inferenceSettlementAuthority = {
  ...
  releaseUnbound: async ({ reservationId, principalId: requestedPrincipalId, projectId: requestedProjectId }) => {
    ...
    assert.equal(requestedProjectId, projectId);
    freshReleased = true;
    return { authorized: true, released: true, outcome: "released", reservationId, principalId: requestedPrincipalId, projectId: requestedProjectId };
  },
};

await assert.rejects(
  () => runMovieMentorTurn(
    { projectId, creatorSessionId: durable.creatorSessionId, creatorTurnId, message: "Race the same creator turn while another runtime wins execution creation." },
    {
      serverAuthority: { authenticated: true, projectAuthorized: true, principalId, projectId },
      inferenceSpendAuthority, inferenceExecutionAuthority, inferenceSettlementAuthority,
      ...
      readAuthoritativeCreatorState: async () => ({ generation: durable.creatorStateGeneration, fingerprint: durable.creatorStateFingerprint, authorityReference: durable.creatorAuthorityReference, snapshotReference: durable.snapshotReference }),
      orchestrateTurn: async () => { orchestrationCalls += 1; throw new Error("invalid winning reservation must stop before orchestration"); },
    },
  ),
