...
const authority=createMovieMentorCommercialCheckoutInitiationAuthority({
 ...
 revokeProviderCheckout:async({provider,checkoutReference})=>{revokeCalls++;assert.equal(provider,"provider-a");assert.equal(checkoutReference,"checkout_post_provider_race_1");return Object.freeze({revoked:true,provider,checkoutReference,status:"expired"});}
});
...
assert.equal(revokeCalls,1,"the court's production-capable provider must revoke the charge-capable checkout before failing closed");
