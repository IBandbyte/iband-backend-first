...
const commercialIntentId="intent-reversal-1";
const checkoutReference="cs_reversal_1";
const providerPaymentReference="pi_reversal_1";
...
const checkoutBindingAuthority={
 resolveCheckoutBinding:async({commercialIntentId:id})=>id===commercialIntentId?Object.freeze({commercialIntentId,provider:"stripe",status:"completed",checkoutReference,providerPaymentReference:boundPaymentReference}):null,
 bindProviderPaymentReference:async({commercialIntentId:id,checkoutReference:ref,providerPaymentReference:paymentRef})=>{assert.equal(id,commercialIntentId);assert.equal(ref,checkoutReference);boundPaymentReference=paymentRef;return Object.freeze({commercialIntentId,provider:"stripe",status:"completed",checkoutReference,providerPaymentReference:paymentRef});},
 resolveCheckoutBindingByProviderPaymentReference:async({provider,paymentReference})=>provider==="stripe"&&paymentReference===boundPaymentReference?Object.freeze({commercialIntentId,provider:"stripe",status:"completed",checkoutReference,providerPaymentReference:paymentReference}):null,
...
assert.equal(issuanceCalls,1,"paid checkout must still reach entitlement issuance");
assert.equal(boundPaymentReference,providerPaymentReference,"successful payment evidence must durably bind provider payment identity to the exact checkout session before reversal can borrow it");
...
assert.equal(reversalCalls,1,"verified refund must cross durable reversal authority exactly once");
assert.equal(reversed?.suspended,true,"verified refund must suspend forward entitlement authority pending reconciliation");
