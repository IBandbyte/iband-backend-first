...
console.log("Movie Mentor consumption entitlement CAS return authority court");
...
const counterfeitEntitlement={
  domain:"iband.movie-mentor.inference-spend",schema:1,principalId:"OTHER-CREATOR",
  status:"suspended",remainingUnits:-99,reservedUnits:999,consumedUnits:-7,entitlementRevision:0,
};
...
const collection=name=>{
  ...
  if(name==="movie_mentor_inference_entitlement")return {findOneAndUpdate:async()=>structuredClone(counterfeitEntitlement)};
  return {};
};
...
await assert.rejects(
  ...
  e=>e?.code==="MOVIE_MENTOR_INFERENCE_SETTLEMENT_LEDGER_CONFLICT",
  "successful consumption entitlement CAS return must bind exact principal, spend schema/domain, active state and plausible post-consumption ledger state"
);

console.log("GREEN: consumption entitlement CAS return binds the durable post-consumption entitlement universe.");
