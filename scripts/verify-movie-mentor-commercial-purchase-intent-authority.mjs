...
const records=new Map();const attempts=new Map();const store={async create(r){const out=Object.freeze({...r,status:"created",createdAt:new Date().toISOString()});records.set(r.commercialIntentId,out);attempts.set(`${r.principalId}:${r.purchaseAttemptDigest}`,out);return out;},async resolve({commercialIntentId}){return records.get(commercialIntentId)||null;},async resolveAttempt({principalId,purchaseAttemptDigest}){return attempts.get(`${principalId}:${purchaseAttemptDigest}`)||null;}};
...
await assert.rejects(()=>authority.createPurchaseIntent({principalId:"",packageId:"creator-20",purchaseAttemptId:"attempt_invalid"}),e=>e.code==="MOVIE_MENTOR_PURCHASE_INTENT_REQUEST_INVALID");
await assert.rejects(()=>authority.createPurchaseIntent({principalId:"principal_A",packageId:"invented",purchaseAttemptId:"attempt_invented"}),e=>e.code==="MOVIE_MENTOR_PURCHASE_INTENT_POLICY_INVALID");
const intent=await authority.createPurchaseIntent({principalId:"principal_A",packageId:"creator-20",purchaseAttemptId:"attempt_creator20_1",units:999999,amountMinor:1,currency:"USD"});
...
console.log("✓ direct purchase-intent court owns purchaseAttemptId and durable attempt-recovery fixture");
console.log("✓ production purchase-intent composition exports the capability proof it owns");
console.log("PASS Door 5A.9 — authenticated principal plus package request and purchase attempt creates durable server-owned immutable commercial intent; creator cannot dictate price/units; later policy cannot rewrite an existing purchase; 5A.7 verifies against the frozen intent snapshot.");
