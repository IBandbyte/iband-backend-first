...
const signedPaidEvent=Object.freeze({id:"evt_checkout_session_binding",type:"checkout.session.completed",livemode:true,data:{object:{id:rogueCheckoutReference,client_reference_id:commercialIntentId,metadata:{commercialIntentId,providerProductId:"price-20"},payment_status:"paid",amount_total:1200,currency:"gbp"}}});
const stripe={checkout:{sessions:{create:async()=>{throw new Error("checkout creation is outside this payment-evidence court");}}},webhooks:{constructEvent(){return signedPaidEvent;}}};
...
assert.equal(normalized.checkoutReference,rogueCheckoutReference);
assert.equal(normalized.commerciallyFinal,true);
assert.notEqual(rogueCheckoutReference,authorizedCheckoutReference);

const ingress=createMovieMentorCommercialProviderIngressAuthority({providers:{stripe:adapter},purchaseIntentAuthority,checkoutBindingAuthority,issuanceAuthority});
await assert.rejects(()=>ingress.processProviderDelivery({provider:"stripe",delivery}),error=>error?.code==="MOVIE_MENTOR_COMMERCIAL_EVIDENCE_CHECKOUT_BINDING_MISMATCH","A signed paid provider event from a different checkout session must not borrow the purchase intent's entitlement authority.");
