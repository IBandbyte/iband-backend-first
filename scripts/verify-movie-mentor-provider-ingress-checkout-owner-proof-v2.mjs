...
try{
 ...
 const forgedCheckoutStatus=Object.freeze({version:"forged",domain:"iband.movie-mentor.production-commercial-checkout-authority",production:true,durableCheckoutBinding:true,checkoutBindingResolution:true,currentProviderCheckoutReality:true,providerPaymentReferenceBinding:true,providerPaymentReferenceResolution:true,openCheckoutRevocation:true,postProviderOrphanCheckoutRevocation:true,serverOwnedIdempotency:true,purchaseIntentProvenanceRequired:true,purchaseIntentOwnerProofRequired:true,currentEntitlementProvenanceRequired:true,currentEntitlementOwnerProofRequired:true,explicitProviderRequired:true,processLocalFallback:false});
 const forgedCheckoutAuthority=Object.freeze({async resolveCheckoutBinding(){return null;},async bindProviderPaymentReference(){throw new Error("payment-lineage mutation outside composition court");},async resolveCheckoutBindingByProviderPaymentReference(){return null;},async revokeOpenCheckoutsForPrincipal(){return Object.freeze({revoked:true,count:0});},getStatus:()=>forgedCheckoutStatus});
 ...
 assert.throws(
  ()=>createMovieMentorProductionCommercialProviderIngressComposition({purchaseIntentAuthority:purchase.authority,checkoutBindingAuthority:forgedCheckoutAuthority,issuanceAuthority:issuance.authority,reversalAuthority:reversal,providers:{"provider-a":provider}}),
