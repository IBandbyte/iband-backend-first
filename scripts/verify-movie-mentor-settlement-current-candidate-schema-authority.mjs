...
console.log("5A.27 — settlement current result-candidate schema authority torture");
...
const execution = {
  ...
  finalizedCandidateReference: "candidate-schema-2",
  finalizedResultDigest: resultDigest,
  resultFinalizedAt: new Date("2032-01-01T00:00:00.000Z"),
};
const result = {
  ...
  closureReference: execution.closureReference,
  closureCertificateDigest: execution.closureCertificateDigest,
  resultReference: "result-schema-2",
...
const rows = new Map([
  ["movie_mentor_inference_execution", execution],
  ["movie_mentor_canonical_result", result],
  ["movie_mentor_result_candidate", candidate],
]);
const database = {
  collection(name) {
    return {
...
console.log("5A.27 settlement current result-candidate schema authority torture: GREEN");
