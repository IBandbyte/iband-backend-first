...
const candidateStatus=getMovieMentorResultCandidateMongoStoreStatus();assert.equal(candidateStatus.schema,2);
const payload={success:true,mentorResponse:{text:"Current candidate reality must survive canonicalization."}},resultDigest=digest(payload);
const record={resultReference:"canonical-result-schema-2",candidateReference:"candidate-schema-2",executionId:"execution-schema-2",creatorTurnId:"turn-schema-2",principalId:"creator-schema-2",projectId:"project-schema-2",reservationId:"reservation-schema-2",requestDigest:"request-schema-2",closureReference:"closure-schema-2",closureCertificateDigest:"closure-digest-schema-2",resultDigest,resultPayload:stable(payload),committedAt:"2032-01-01T00:00:00.000Z"};
let canonicalRow=null;const model={findOne(query){return{session(){return this;},lean(){return this;},async exec(){if(!canonicalRow)return null;if(query.executionId&&canonicalRow.executionId!==query.executionId)return null;if(query.principalId&&canonicalRow.principalId!==query.principalId)return null;if(query.projectId&&canonicalRow.projectId!==query.projectId)return null;if(query.creatorTurnId&&canonicalRow.creatorTurnId!==query.creatorTurnId)return null;return structuredClone(canonicalRow);}};},async create(rows){canonicalRow=structuredClone(rows[0]);return[structuredClone(canonicalRow)];}};
let executionRow={domain:"iband.movie-mentor.inference-execution-store",schema:6,phase:"closed",providerEffectRealityRevision:7,executionId:record.executionId,creatorTurnId:record.creatorTurnId,principalId:record.principalId,projectId:record.projectId,reservationId:record.reservationId,requestDigest:record.requestDigest,leaseGeneration:8,leaseReference:"lease-schema-2",fencingToken:"fence-schema-2",closureReference:record.closureReference,closureCertificateDigest:record.closureCertificateDigest,resultFinalizationBarrierRevision:0};
