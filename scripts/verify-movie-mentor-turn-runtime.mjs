...
const serverAuthority={authenticated:true,projectAuthorized:true,principalId:"creator-77",projectId:"project-77"};
const inferenceSpendAuthority={reserveTurn:async({serverAuthority:authority,projectId})=>{assert.equal(authority.principalId,"creator-77");assert.equal(projectId,"project-77");return{authorized:true,reservationId:"runtime-test-reservation",operation:"movie-mentor-turn",units:1};}};
...
const result=await runMovieMentorTurn({projectId:"project-77",creatorSessionId:"session-9",message:"The lighthouse answers tonight.",authoritativeTurnContext:{domain:"purple.universe"},context:forged},{
 serverAuthority,inferenceSpendAuthority,
...
await assert.rejects(()=>runMovieMentorTurn({projectId:"project-77",message:"hello"},{serverAuthority,inferenceSpendAuthority,readAuthoritativeTurnSource:async()=>{const e=new Error("missing");e.code="MOVIE_MENTOR_CREATOR_STATE_NOT_FOUND";throw e;}}),e=>e.code==="MOVIE_MENTOR_CREATOR_STATE_NOT_FOUND");
await assert.rejects(()=>runMovieMentorTurn({projectId:"project-77",message:"hello"},{readAuthoritativeTurnSource:async()=>structuredClone(durable),orchestrateTurn:async()=>({})}),e=>e.code==="MOVIE_MENTOR_INFERENCE_SPEND_AUTHORITY_REQUIRED");
...
let revisionReads=0;await runMovieMentorTurn({projectId:"project-77",message:"hello"},{serverAuthority,inferenceSpendAuthority,readAuthoritativeTurnSource:async()=>structuredClone(durable),readAuthoritativeRevision:async()=>{revisionReads++;return{revision:43,reference:"revision:project-77:43"}},readAuthoritativeCreatorState:async()=>({generation:8,fingerprint:"b".repeat(64),authorityReference:"creator-state:project-77:g8"}),orchestrateTurn:async(input,deps)=>{assert.equal((await deps.readAuthoritativeRevision({projectId:"project-77"})).revision,43);assert.equal((await deps.readAuthoritativeCreatorState({projectId:"project-77"})).generation,8);return{success:true,text:"test"};}});assert.equal(revisionReads,1);

console.log("Movie Mentor Turn Runtime v1.2 passed: canonical durable creator state remains authoritative, forged client context stays inert, and a server-created durable inference-spend reservation is required before orchestration.");
