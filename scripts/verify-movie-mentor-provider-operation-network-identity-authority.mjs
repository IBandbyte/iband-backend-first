...
try{
 ...
 assert.equal(shared.result.structured.value,"ok");assert.equal(shared.header,operation.providerOperationId);
 ...
 assert.equal(semantic.result.structured.movieJourneyIntelligence.readyToAdvance,true);assert.equal(semantic.header,operation.providerOperationId);
 ...
 assert.equal(specialist.result.success,true);assert.equal(specialist.header,operation.providerOperationId);
 ...
 assert.equal(continuity.result.success,true);assert.equal(continuity.header,operation.providerOperationId);
 ...
 assert.equal(synthesis.result.success,true);assert.equal(synthesis.header,operation.providerOperationId);
 let runtimeOperation=null,inputBoundBeforeUnknown=false;
 ...
 const runtimeAuthority={async claimProviderCall(){return providerCall;},async bindProviderReconstructionInput({providerCall:call,reconstructionInput}={}){assert.equal(call,providerCall);assert.deepEqual(reconstructionInput,{proof:true});inputBoundBeforeUnknown=true;return{authorized:true,inputBound:true};},async beginProviderDispatch(){assert.equal(inputBoundBeforeUnknown,true,"runtime must bind exact task input before UNKNOWN");return{authorized:true,dispatchAuthorized:true,effectState:"unknown"};},async assertProviderDispatch(){return{authorized:true,dispatchAuthorized:true};},async contributeProviderEffectEvidence(){return{accepted:true,state:"confirmed"};}};
 ...
 assert.deepEqual(runtimeOperation,{providerOperationId:providerCall.providerCallId,executionId:providerCall.executionId,slotId:providerCall.slotId,task:providerCall.task});
 assert.equal(inputBoundBeforeUnknown,true);
 ...
 console.log("✓ fenced runtime binds exact task input before UNKNOWN and transports exact provider-call identity into the provider adapter boundary");
 ...
 console.log("LAW: NETWORK IDENTITY AND HISTORICAL INPUT ARE SEPARATE AUTHORITIES; LIVE DISPATCH REQUIRES BOTH.");
