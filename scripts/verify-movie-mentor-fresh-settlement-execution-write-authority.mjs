...
console.log("Movie Mentor fresh settlement execution write authority court");
...
const settledReservation={...reservation,status:"consumed",settledAt:"2036-01-02T00:00:00.000Z",settlementReason:"canonical-result:result-A",settlementExecutionId:"execution-A",settlementResultReference:"result-A",settlementCandidateReference:"candidate-A",settlementResultDigest:resultDigest};
const collection=name=>{
 ...
 if(name==="movie_mentor_inference_spend_reservation")return {findOne:async()=>structuredClone(reservation),findOneAndUpdate:async()=>structuredClone(settledReservation)};
 if(name==="movie_mentor_inference_entitlement")return {findOneAndUpdate:async()=>structuredClone(entitlement)};
 return {};
};
...
await assert.rejects(()=>store.settleCanonicalResult({executionId:"execution-A"}),e=>e?.code==="MOVIE_MENTOR_INFERENCE_SETTLEMENT_REALITY_RACE","successful fresh finalized-to-settled execution barrier must prove durable execution phase and exact canonical lineage rather than trusting matchedCount alone");
console.log("GREEN: fresh finalized-to-settled execution barrier is durably revalidated.");
