...
let providerCalls=0;
const durableEntitlement=Object.freeze({principalId:"creator_suspended",status:"suspended",remainingUnits:7,reservedUnits:0,consumedUnits:3,entitlementRevision:11});
...
const authority=createMovieMentorCommercialCheckoutInitiationAuthority({resolvePurchaseIntent:async()=>intent,resolveCurrentEntitlement:async({principalId})=>principalId===durableEntitlement.principalId?durableEntitlement:null,createProviderCheckout:async()=>{providerCalls++;return Object.freeze({authorized:true,commercialIntentId:intent.commercialIntentId,provider:"provider-a",checkoutReference:"checkout_suspended_1",checkoutUrl:"https://provider.example/checkout_suspended_1",expiresAt:null});},checkoutBindingStore:bindingStore});
