...
candidate = replaceExact(candidate, 'const VERSION="1.2.0"', 'const VERSION="1.3.0"', "candidate version");
...
candidate = replaceExact(candidate, 'stagedAt:new Date(now())', 'stagedAt:instant(now())', "candidate write clock");
...
canonicalAuthority = replaceExact(canonicalAuthority, 'const VERSION="1.5.0"', 'const VERSION="1.6.0"', "canonical authority version");
canonicalAuthority = replaceExact(
  canonicalAuthority,
...
canonicalAuthority = replaceExact(canonicalAuthority, 'existing?.committedAt||new Date(now()).toISOString()', 'existing?.committedAt||instant(now()).toISOString()', "canonical write clock");
...
settlement = replaceExact(settlement, 'const VERSION="1.6.0"', 'const VERSION="1.7.0"', "settlement version");
...
if (clockCount < 1) throw new Error("settlement write clock anchor missing");
settlement = settlement.replaceAll('new Date(now())', 'settlementInstant(now())');
