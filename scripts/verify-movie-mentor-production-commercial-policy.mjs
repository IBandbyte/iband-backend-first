...
const configurationSource="MOVIE_MENTOR_COMMERCIAL_POLICY_JSON";
const registry=createMovieMentorCommercialPolicyRegistry({policies:[p1],configurationSource});
...
assert.equal(registry.catalogueAuthority.getStatus().configurationSource,configurationSource);

assert.throws(()=>createMovieMentorCommercialPolicyRegistry({policies:[p1,{...p1,providerProductId:"different"}],configurationSource}),e=>e?.code==="MOVIE_MENTOR_COMMERCIAL_POLICY_DUPLICATE_PACKAGE");
for(const bad of [{...p1,amountMinor:0},{...p1,units:0},{...p1,currency:"£"},{...p1,provider:""},{...p1,policyVersion:""}])assert.throws(()=>createMovieMentorCommercialPolicyRegistry({policies:[bad],configurationSource}),e=>e?.code==="MOVIE_MENTOR_COMMERCIAL_POLICY_INVALID");
assert.throws(()=>createMovieMentorCommercialPolicyRegistry({policies:[p1]}),e=>e?.code==="MOVIE_MENTOR_COMMERCIAL_POLICY_CONFIGURATION_SOURCE_REQUIRED");
...
console.log("✓ catalogue authority requires explicit server-owned configuration-source provenance");
