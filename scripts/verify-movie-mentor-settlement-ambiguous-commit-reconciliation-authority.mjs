...
console.log("Movie Mentor settlement ambiguous commit reconciliation authority court");
...
const settledAt="2036-01-02T00:00:00.000Z";
...
const ambiguous=Object.assign(new Error("commit result unknown"),{errorLabels:["UnknownTransactionCommitResult"],hasErrorLabel(label){return this.errorLabels.includes(label)}});
const session={async withTransaction(fn){transactionAttempts+=1;await fn();throw ambiguous},async endSession(){}};
...
assert.equal(outcome?.settled,true,"ambiguous commit must reconcile durable settled reality instead of returning generic unavailable after durable commit");
assert.equal(outcome?.authorized,true);
assert.equal(outcome?.idempotent,true,"reconciled ambiguous commit must be identified as durable replay/reconciliation");
assert.equal(transactionAttempts,1);
console.log("GREEN: ambiguous settlement commit reconciles exact durable settled reality before returning authority.");
