...
async function attempt(executionSchema){
  ...
  const fakeModel={collection:{async indexes(){return[{name:"providerCallId_1",key:{providerCallId:1},unique:true}];}},findOne(){return query();},async create(rows){effectRow=structuredClone(rows[0]);return[structuredClone(effectRow)];}};
...
console.log("GREEN: provider-effect UNKNOWN independently requires physical providerCallId uniqueness and the current durable execution schema at its own irreversible boundary.");
