...
assert.match(release,/candidateBarrierRevision=Number\.isSafeInteger\(execution\.resultCandidateBarrierRevision\)\?execution\.resultCandidateBarrierRevision:0/,"release must snapshot the candidate barrier revision it proves");\nassert.match(release,/providerEffectRealityRevision:realityRevision,resultCandidateBarrierRevision:candidateBarrierRevision/,"predispatch-abandonment abort must CAS candidate barrier");\nassert.match(release,/"providerCalls\\.0":\\{\\$exists:false\\},resultCandidateBarrierRevision:candidateBarrierRevision/,"zero-claim abort must CAS candidate barrier");
