...
try {
  ...
  const structuredError = await captureFailure(() => executeStructuredAI({
    ...
    input: { proof: true },
    schema: { type: "object", additionalProperties: false, properties: { requiredValue: { type: "string" } }, required: ["requiredValue"] },
    schemaName: "movie_mentor_provider_response_evidence_proof",
  }));
  assert.equal(networkCalls, 1);
  assert.equal(structuredError?.code, "AI_PROVIDER_STRUCTURED_OUTPUT_SCHEMA_INVALID");
  assert.deepEqual(structuredError?.providerEffectEvidence, { externalEffectId: "resp-structured-known", provider: "openai" });
  ...
  assert.equal(semanticError?.code, "SEMANTIC_STRUCTURED_OUTPUT_INVALID");
  assert.deepEqual(semanticError?.providerEffectEvidence, { externalEffectId: "resp-semantic-known", provider: "openai" });

  const durableEvidence = [];
  const inputBindings = [];
  const providerCall = Object.freeze({
    authorized: true, dispatchAuthorized: true, projectId: "project-runtime-proof", principalId: "creator-runtime-proof",
    creatorTurnId: "turn-runtime-proof", reservationId: "reservation-runtime-proof", requestDigest: "request-runtime-proof",
    providerCallId: "provider-call-runtime-proof", executionId: "execution-runtime-proof", slotId: "semantic",
    task: "movie-mentor-semantic", ownerId: "worker-runtime-proof", leaseGeneration: 1,
    leaseReference: "lease-runtime-proof", fencingToken: "fence-runtime-proof", admittedAt: "2031-01-01T00:00:00.000Z",
  });
  const runtimeAuthority = {
    ...
    async bindProviderReconstructionInput({ providerCall: call, reconstructionInput } = {}) {
      inputBindings.push({ providerCallId: call?.providerCallId, reconstructionInput: structuredClone(reconstructionInput) });
      return { authorized: true, inputBound: true };
    },
    async beginProviderDispatch() {
      assert.equal(inputBindings.length, 1, "historical task input must be durable before UNKNOWN begins");
  ...
  assert.deepEqual(inputBindings, [{ providerCallId: "provider-call-runtime-proof", reconstructionInput: { proof: true } }]);
  assert.deepEqual(durableEvidence, [{ providerCallId: "provider-call-runtime-proof", externalEffectId: "resp-runtime-known", provider: "openai", source: "provider-error-evidence" }]);
  ...
  console.log("✓ runtime binds historical input before UNKNOWN, then converts preserved error evidence into one durable effect contribution");
  ...
  console.log("LAW: PROVIDER EFFECT EVIDENCE DOES NOT EXCUSE MISSING PRE-UNKNOWN HISTORICAL INPUT AUTHORITY.");
