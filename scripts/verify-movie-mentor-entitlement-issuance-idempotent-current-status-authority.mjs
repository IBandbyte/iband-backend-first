...
async function attempt(entitlementStatus){
 ...
 const I={findOne:()=>query(receipt)};
 const E={findOne(){entitlementReads+=1;return query({domain:"iband.movie-mentor.inference-spend",schema:1,principalId:evidence.principalId,status:entitlementStatus,remainingUnits:20,reservedUnits:0,consumedUnits:0,entitlementRevision:3});}};
...
console.log("GREEN: idempotent entitlement issuance replay revalidates current durable entitlement status.");
