...
const effect={providerCallId:"call-342",executionId:"execution-342",slotId:"semantic",task:"movie-mentor-semantic",state:"confirmed",revision:1,evidence:[{externalEffectId:"effect-342",provider:"test",observedAt:"2035-01-01T00:00:01.000Z",source:"provider-ack"}]};
...
assert.notEqual(result.authorized,true,"Creator state advancing after proof must prevent compensation from committing against the stale universe");
assert.equal(rows.movie_mentor_inference_entitlement.remainingUnits,4,"stale Creator-state proof must restore zero units");
assert.equal(rows.movie_mentor_inference_spend_reservation.status,"reserved","stale Creator-state proof must preserve reservation");
