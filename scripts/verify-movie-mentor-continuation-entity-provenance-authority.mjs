...
await assert.rejects(()=>applyMovieMentorCreatorStateTransition({projectId,source:"creator-memory",expectedRevision:11,state:{memoryContext:forged}},{readAuthoritativeTurnSource:async()=>structuredClone(durable),writeAuthoritativeCreatorState:async next=>{writes+=1;persisted=structuredClone(next);return persisted;},creatorStateMutationAuthority:{assertCurrentMutation:async()=>({authorized:true})}}),error=>error?.code==="MOVIE_MENTOR_CONTINUATION_ENTITY_PROVENANCE_REQUIRED");
