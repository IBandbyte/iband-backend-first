...
console.log("Movie Mentor settlement transient transaction retry authority court");
...
assert.equal(transactionAttempts,2,"transient transaction must re-enter settlement callback");
assert.equal(outcome?.settled,true);
assert.equal(outcome?.authorized,true);
assert.equal(outcome?.outcome,"consumed");
assert.equal(outcome?.executionId,"execution-A");
assert.equal(outcome?.reservationId,"reservation-A");
console.log("GREEN: transient settlement transaction retry re-enters from current durable authority and settles exactly once.");
