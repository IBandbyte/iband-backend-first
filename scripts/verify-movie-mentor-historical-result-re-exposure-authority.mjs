...
const settlement={authorized:true,settled:true,outcome:"consumed",resultFinalizationVerified:true,executionPhase:"settled",providerEffectRealityRevision:7,executionId:"execution-1",principalId:"creator-1",projectId:"project-1",reservationId:"reservation-1",resultReference:"result-1",candidateReference:"candidate-1",resultDigest:canonical.resultDigest,closureCertificateDigest:"closure-digest-1"};
const runtimeResult=()=>({...structuredClone(payload),metadata:{...structuredClone(payload.metadata),canonicalResult:{authorized:true,currentRealityVerified:true,candidateLineageVerified:true,resultFinalizationVerified:true,creatorResponseAuthorityVerified:true,resultReference:"result-1",candidateReference:"candidate-1",resultDigest:canonical.resultDigest,executionId:"execution-1",closureReference:"closure-1",closureCertificateDigest:"closure-digest-1",reservationId:"reservation-1",settlement:"consumed",settlementExecutionPhase:"settled",replayedFromDurableResult:true}}});
...
console.log("✓ historical replay fixture carries the exact canonical candidate lineage through runtime and settlement proofs");
