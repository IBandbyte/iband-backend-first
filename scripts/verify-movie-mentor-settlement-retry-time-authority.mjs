...
console.log("Movie Mentor settlement retry time authority court");
...
assert.equal(execution.settledAt,secondAttemptAt,"retried transaction must derive settlement time inside the re-entered transaction attempt");
assert.equal(reservation.settledAt,secondAttemptAt,"reservation settlement time must bind the successful retry, not the rolled-back attempt");
console.log("GREEN: retried settlement derives settlement time from the successful transaction attempt, not rolled-back process history.");
