...
async function attempt(schema){
  ...
  const executionCollection={async updateOne(filter){executionBarrierFilter=structuredClone(filter);return{matchedCount:filter.schema===schema?1:0};}};
  const creatorStateCollection={async updateOne(){return{matchedCount:1};}};
  const store=createMovieMentorResultCandidateMongoStore({mongoModel:candidateModel(),executionCollection,creatorStateCollection,startSession:async()=>session(),now:()=>new Date("2035-01-01T00:00:00.000Z"),randomId:()=>`schema-${schema}`});
  const execution={...executionBase,schema};
