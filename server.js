...
async function mountRoute(basePath, importPath) {
  try {
    ...
    console.log(`[mount:skip] ${basePath} -> ${importPath} (${error?.code || "load_error"})`);
  }
}

app.get("/", (req, res) => res.json({ success: true, service: "iband-backend-first", app: "iBand", platform: "iBandbyte", company: "iBandbyte Ltd", environment: NODE_ENV, version: "movie-mentor-specialist-agents", message: "iBand backend is live.", now: new Date().toISOString() }));
app.get("/health", (req, res) => res.json({ success: true, status: "ok", uptimeSec: Math.floor(process.uptime()), now: new Date().toISOString() }));
app.get("/api", (req, res) => res.json({ success: true, message: "iBand API root", modules: ["smart-feed","personalised-feed","feed-diversity","engagement-optimiser","session-learning","predictive-feed","movie-mentor-semantic","movie-mentor-specialists"] }));
...
async function startServer() {
  ...
  await mountRoute("/api/movie-mentor-specialists", "./movieMentorSpecialists.js");

  app.use((req, res) => res.status(404).json({ success: false, message: "Route not found" }));
  app.listen(PORT, () => console.log(`[boot] iband-backend-first listening on port ${PORT}`));
}
