...
import { assertMovieMentorForwardExecutionReacquisitionAuthority } from "./MovieMentorForwardExecutionAuthority.js";

const MOVIE_MENTOR_FORWARD_EXECUTION_RUNTIME_VERSION = "1.0.0";
...
function createForwardExecutionRuntimeDeps(deps = {}) {
  ...
  if (typeof authority?.assertCurrentReacquisition !== "function") throw Object.assign(new Error("Forward execution runtime requires server-created current ownership authority."), { code: "MOVIE_MENTOR_FORWARD_EXECUTION_AUTHORITY_REQUIRED" });

  const historical = new Map();
  const guarded = Object.create(base);
