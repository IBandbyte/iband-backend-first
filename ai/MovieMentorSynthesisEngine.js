...
const MOVIE_MENTOR_SYNTHESIS_VERSION="1.0.0";
...
async function synthesizeMovieMentorResponse(input={}){
 ...
 const raw=await executeStructuredAI({task:"movie-mentor-synthesis",systemInstructions:SYNTHESIS_RULES,input:{creatorMessage:cleanString(input.creatorMessage),creatorConfirmedContext:clone(asArray(input.creatorConfirmedContext)),semanticIntelligence:clone(input.semanticIntelligence||{}),semanticMentorDraft:cleanString(input.semanticMentorDraft)||null,contributions:clone(asArray(input.contributions)),responseBlueprint:clone(input.responseBlueprint||null),communicationPlan:clone(input.communicationPlan||null)},schema:SYNTHESIS_SCHEMA,schemaName:"movie_mentor_synthesis",metadata:{synthesisVersion:MOVIE_MENTOR_SYNTHESIS_VERSION,contractVersion:MENTOR_SYNTHESIS_CONTRACT_VERSION}});
 ...
 const invalidIds=[...used,...deferred].filter(id=>!contributionIds.has(id));
...
export {MOVIE_MENTOR_SYNTHESIS_VERSION,MENTOR_SYNTHESIS_CONTRACT_VERSION,SYNTHESIS_SCHEMA,materialClarificationRequired,validateSynthesisRequest,synthesizeMovieMentorResponse};
