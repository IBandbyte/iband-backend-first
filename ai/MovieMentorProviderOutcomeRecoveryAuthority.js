...
function createMovieMentorProviderOutcomeRecoveryAuthority({
  ...
  if (typeof readProviderOperation !== "function" || typeof readProviderEffectReality !== "function") {
    fail("MOVIE_MENTOR_PROVIDER_OUTCOME_DURABLE_READERS_REQUIRED", "Provider outcome recovery requires durable provider operation and provider-effect reality readers.");
  }
  if (typeof recoverProviderResponse !== "function") {
    fail("MOVIE_MENTOR_PROVIDER_OUTCOME_RECOVERY_ADAPTER_REQUIRED", "Provider outcome recovery requires an explicit provider recovery adapter.");
  }
  if (typeof resolveCurrentTarget !== "function") {
    fail("MOVIE_MENTOR_PROVIDER_OUTCOME_TARGET_RESOLVER_REQUIRED", "Provider outcome recovery requires a current provider target resolver.");
  }
  if (typeof requireRecoveryAuthority !== "boolean") {
    fail("MOVIE_MENTOR_PROVIDER_OUTCOME_RECOVERY_AUTHORITY_CONFIGURATION_INVALID", "Provider outcome recovery authority requirement must be an explicit boolean.");
  }
  if (requireRecoveryAuthority && typeof assertCurrentRecoveryAuthority !== "function") {
    fail("MOVIE_MENTOR_PROVIDER_OUTCOME_RECOVERY_AUTHORITY_CALLBACK_REQUIRED", "Production provider outcome recovery requires a current recovery-authority callback.");
  }
  ...
  async function reconcile({ providerCallId = null, recoveryAuthority = null } = {}) {
    ...
    if (!operation) {
      return freeze({ authorized: false, domain: DOMAIN, outcome: OUTCOMES.STILL_UNKNOWN, providerCallId: callId, providerOperationId: callId, recoveryAuthorized: false, recovered: false, redispatchAuthorized: false, refundAuthorized: false, reason: "provider-operation-not-found" });
    }

    const binding = operationBinding(operation);
    if (binding.providerCallId !== callId) {
      fail("MOVIE_MENTOR_PROVIDER_OUTCOME_OPERATION_IDENTITY_CONFLICT", "Recovered provider operation identity does not match the requested historical provider call.");
    }
    ...
    if (state === "unknown") {
      if (evidence.length !== 0) return conflictingEffect(binding, operation, { reason: "unknown-provider-effect-carries-evidence" });
    ...
    if (evidence[0].provider !== providerTarget.provider) {
      return conflictingEffect(binding, operation, { reason: "provider-effect-provider-mismatch", evidenceProvider: evidence[0].provider });
    }

    if (providerTarget.recoveryMode !== "known-response-id-retrieval") {
      return confirmedEffect(binding, operation, externalEffectId, { recoveryAuthorized: false, recovered: false, reason: "provider-recovery-capability-not-declared" });
    }
    ...
    if (!sameMovieMentorProviderTarget(providerTarget, currentTarget)) {
      return confirmedEffect(binding, operation, externalEffectId, { recoveryAuthorized: false, recovered: false, reason: "provider-target-no-longer-current", currentProviderTarget: currentTarget });
    }
    ...
    if (requireRecoveryAuthority) {
      ...
      if (!recoveryProof) {
        return confirmedEffect(binding, operation, externalEffectId, { recoveryAuthorized: false, recovered: false, reason: "provider-recovery-execution-fenced", recoveryAuthorityReason: text(proof?.reason) || null });
      }
    }
    ...
    if (
      ...
      || (recoveredOperationId && recoveredOperationId !== binding.providerCallId)
      || (requireRecoveryAuthority && recoveredOperationId !== binding.providerCallId)
      || !recovered.response
      || typeof recovered.response !== "object"
    ) {
      return conflictingEffect(binding, operation, {
        reason: recoveredOperationId && recoveredOperationId !== binding.providerCallId
          ? "provider-recovery-operation-binding-conflict"
          : "provider-recovery-response-binding-invalid",
        ...(recoveredOperationId ? { recoveredProviderOperationId: recoveredOperationId } : {}),
      });
    }
    const responseId = text(recovered.response.id);
    if (responseId && responseId !== externalEffectId) {
      return conflictingEffect(binding, operation, { reason: "provider-recovery-response-id-conflict", recoveredResponseId: responseId, externalEffectId });
    }
