...
function createMovieMentorLegacyMigrationAttestationIssuer({now=()=>Date.now(),randomId=()=>crypto.randomUUID(),issuer="iband.movie-mentor.legacy-migration-authority",ttlMs=10*60*1000,maxAdoptionIdAttempts=4,readConsumption=null,bindConsumptionIssuance=null,createIssuance=null,readIssuanceByConsumptionId=null}={}){
  if(typeof readConsumption!=="function"||typeof bindConsumptionIssuance!=="function"||typeof createIssuance!=="function"||typeof readIssuanceByConsumptionId!=="function") fail("MOVIE_MENTOR_LEGACY_ATTESTATION_DURABILITY_REQUIRED","Attestation issuance requires durable consumption, issuance-lineage, and issuance authorities.");
  async function bindReality(cid,record,status){const adoptionId=s(record?.adoptionId||record?.attestation?.adoptionId);if(!adoptionId)fail("MOVIE_MENTOR_LEGACY_ATTESTATION_ISSUANCE_INVALID","Durable issuance reality is missing adoption identity.");await bindConsumptionIssuance({consumptionId:cid,adoptionId,boundAt:new Date(Number(now())).toISOString()});return result(status,record);}
  async function issue({consumptionId,principal,project}={}){
    ...
    const existing=await readIssuanceByConsumptionId({consumptionId:cid});
    const lineageAdoptionId=s(consumption.issuanceAdoptionId);
    if(lineageAdoptionId&&!existing) fail("MOVIE_MENTOR_LEGACY_ATTESTATION_ISSUANCE_HISTORY_MISSING","Consumed challenge proves an attestation was previously issued, but the durable issuance is missing. Remint is forbidden.");
    if(existing&&lineageAdoptionId&&s(existing.adoptionId||existing.attestation?.adoptionId)!==lineageAdoptionId) fail("MOVIE_MENTOR_LEGACY_ATTESTATION_ISSUANCE_LINEAGE_CONFLICT","Durable issuance disagrees with consumed-challenge issuance lineage.");
    if(existing)return bindReality(cid,existing,"already-issued");
    for(let attempt=1;attempt<=maxAdoptionIdAttempts;attempt++){
      ...
      try{
        ...
        if(created?.created===true||created?.issued===true){await bindConsumptionIssuance({consumptionId:cid,adoptionId,boundAt:new Date(Number(now())).toISOString()});return Object.freeze({issued:true,status:"issued",attestation});}
        if(created?.conflict==="consumption-already-issued"&&created.record)return bindReality(cid,created.record,"issued-after-race");
        if(created?.conflict==="adoption-id-collision"){
          const reality=await readIssuanceByConsumptionId({consumptionId:cid});
          if(reality)return bindReality(cid,reality,"issued-after-race");
          const latestConsumption=await readConsumption({consumptionId:cid});
          if(s(latestConsumption?.issuanceAdoptionId))fail("MOVIE_MENTOR_LEGACY_ATTESTATION_ISSUANCE_HISTORY_MISSING","Issuance lineage appeared without matching durable issuance. Remint is forbidden.");
          continue;
        }
      }catch(error){
        const reality=await readIssuanceByConsumptionId({consumptionId:cid});
        if(reality)return bindReality(cid,reality,"issued-after-ack-loss");
        throw error;
      }
      const reality=await readIssuanceByConsumptionId({consumptionId:cid}); if(reality)return bindReality(cid,reality,"issued-after-race");
