...
function createMovieMentorLegacyMigrationAttestationIssuer({
  now=()=>Date.now(), randomId=()=>crypto.randomUUID(), issuer="iband.movie-mentor.legacy-migration-authority", ttlMs=10*60*1000,
  readConsumption=null, createIssuance=null, readIssuanceByConsumptionId=null,
}={}){
  ...
  async function issue({consumptionId,principal,project}={}){
    ...
    const identity=consumption.projectIdentity||{}; const currentIdentity=project?.identity||{};
    ...
    if(existing) return Object.freeze({issued:true,status:"already-issued",attestation:freezeAttestation(existing.attestation||existing)});
    const issuedAtMs=Number(now()); const adoptionId=`movie-mentor-legacy-adoption:${randomId()}`;
    ...
    try{
      ...
      // Never mint a replacement after ambiguous failure. Durable reality decides.
      const reality=await readIssuanceByConsumptionId({consumptionId:cid});
      if(reality) return Object.freeze({issued:true,status:"issued-after-ack-loss",attestation:freezeAttestation(reality.attestation||reality)});
      throw error;
    }
    const reality=await readIssuanceByConsumptionId({consumptionId:cid});
    if(reality) return Object.freeze({issued:true,status:"issued-after-race",attestation:freezeAttestation(reality.attestation||reality)});
