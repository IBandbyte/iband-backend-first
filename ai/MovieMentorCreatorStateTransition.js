...
async function applyMovieMentorCreatorStateTransition(input={},deps={}){
 assertNoAuthorityInjection(input);const identity=identityFrom(input);assertIdentity(identity);const source=transitionSource(input);assertCreatorTruthProvenance(input,source,deps);assertAdvisoryReferenceProvenance(input);assertContinuationHistoryProvenance(input);assertContinuationEntityProvenance(input);
