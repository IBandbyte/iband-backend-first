...
const MOVIE_MENTOR_TURN_RUNTIME_VERSION = "1.4.0";
...
function createFencedInferenceOrchestrationDeps({execution,inferenceExecutionAuthority,deps={},onClaim=null}={}){
 ...
 return Object.freeze({
   async interpretSemantics(input){await claim("semantic","movie-mentor-semantic");return interpret(input);},
   async executeSpecialistPlan(plan={}){
     ...
     for(const workOrder of Array.isArray(plan?.workOrders)?plan.workOrders:[]){
       ...
       try{
         await claim(agentId,`movie-mentor-specialist:${agentId}`);
         const result=await executeWorkOrder(clone(workOrder),deps.specialistDeps||{});
     ...
     return {version:MOVIE_MENTOR_SPECIALIST_EXECUTOR_VERSION,contractVersion:SPECIALIST_CONTRACT_VERSION,status:failures.length?"partial":"completed",contributions,skipped,failures,metadata,authority:{providerCallsRequireDurableExecutionClaim:true,creatorTruthDominates:true,specialistsRemainProvisional:true},liveAgents:[...LIVE_AGENT_IDS]};
   },
   async synthesizeResponse(input){await claim("synthesis","movie-mentor-synthesis");return synthesize(input);},
 });
}
...
async function runMovieMentorTurn(input={},deps={}){
 ...
 if(executionEnabled){
   execution=await openLiveExecution({input,creatorMessage,durableProjectId,reservation,serverAuthority:deps.serverAuthority,inferenceExecutionAuthority,deps});
   const fenced=createFencedInferenceOrchestrationDeps({execution,inferenceExecutionAuthority,deps,onClaim:()=>{claimedCount+=1;}});
   orchestrationDeps={...orchestrationDeps,...fenced,verifyTurnContext:deps.verifyTurnContext,resolveContinuationReferences:deps.resolveContinuationReferences,commitCreatorDecision:deps.commitCreatorDecision,readAuthoritativeTurnSource:deps.readAuthoritativeTurnSource,applyMovieMentorCreatorStateTransition:deps.applyMovieMentorCreatorStateTransition,writeAuthoritativeCreatorState:deps.writeAuthoritativeCreatorState};
 }
 ...
 try{
   ...
   if(executionEnabled&&claimedCount>0){
     throw runtimeError("MOVIE_MENTOR_INFERENCE_EXECUTION_UNRESOLVED","Inference execution failed after one or more durable provider-call claims; spend remains reserved for reconciliation.",{cause:error,retryable:true,executionId:execution?.executionId||null,providerCallsClaimed:claimedCount});
   }
   try{await spendAuthority.settleTurn({reservation,outcome:"released",reason:"orchestration-failed-before-provider-claim"});}catch(settlementError){settlementError.cause=error;throw settlementError;}
   throw error;
 }

 const settlement=await spendAuthority.settleTurn({reservation,outcome:"consumed",reason:"orchestration-succeeded"});
 if(settlement?.settled!==true)throw runtimeError("MOVIE_MENTOR_INFERENCE_SPEND_SETTLEMENT_INVALID","Successful Movie Mentor turn requires durable consumed settlement.");
 return{...result,metadata:{...(result?.metadata||{}),inferenceSpend:{authorized:true,reservationId:reservation.reservationId,units:reservation.units,operation:reservation.operation,settlement:"consumed"},...(executionEnabled?{inferenceExecution:{executionId:execution.executionId,creatorTurnId:execution.creatorTurnId,leaseGeneration:execution.leaseGeneration,providerCallsClaimed:claimedCount,liveLeaseEnforced:true}}:{})}};
}

export{MOVIE_MENTOR_TURN_RUNTIME_VERSION,buildRequestDigest,buildTurnEnvelopeFromDurableState,createFencedInferenceOrchestrationDeps,openLiveExecution,runMovieMentorTurn};
