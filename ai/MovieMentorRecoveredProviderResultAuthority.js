...
function assertRecoveredOutcomeBinding({ recovery, historical } = {}) {
  if (recovery?.outcome !== "CONFIRMED_EFFECT" || recovery?.recovered !== true || recovery?.recoveryAuthorized !== true || recovery?.redispatchAuthorized !== false || recovery?.refundAuthorized !== false || !recovery?.recoveredProviderResponse) {
    if (recovery?.outcome === "STILL_UNKNOWN") {
      fail("MOVIE_MENTOR_PROVIDER_RECOVERY_STILL_UNKNOWN", "Historical provider effect remains unknown and cannot be reconstructed or redispatched.", { retryable: true, providerCallId: historical.providerCallId });
    }
    fail("MOVIE_MENTOR_PROVIDER_RECOVERY_RESULT_NOT_AUTHORIZED", "Historical provider outcome did not establish same-operation recovered response authority.", {
      retryable: true, providerCallId: historical.providerCallId, outcome: text(recovery?.outcome) || null, reason: text(recovery?.reason) || null,
    });
  }
  ...
  if (!externalEffectId || !recoveredResponseId || recoveredResponseId !== externalEffectId) {
    fail("MOVIE_MENTOR_PROVIDER_RECOVERY_RESPONSE_EFFECT_BINDING_INVALID", "Recovered provider bytes do not bind the exact authorized historical external effect.", {
      retryable: false, providerCallId: historical.providerCallId, externalEffectId: externalEffectId || null, recoveredResponseId: recoveredResponseId || null,
    });
  }
  return freeze({
...
function assertRecoveryLeaseBinding({ recovery, execution, historical } = {}) {
  ...
  if (!expectedOwnerId || expectedLeaseGeneration === null || !actualOwnerId || actualLeaseGeneration === null || actualOwnerId !== expectedOwnerId || actualLeaseGeneration !== expectedLeaseGeneration) {
    fail("MOVIE_MENTOR_PROVIDER_RECOVERY_LEASE_BINDING_INVALID", "Recovered provider bytes do not belong to the exact current recovery owner and lease generation.", {
      retryable: false,
...
function historicalInputRequired(historical = {}) {
  fail("MOVIE_MENTOR_PROVIDER_RECOVERY_INPUT_AUTHORITY_REQUIRED", "Recovered provider bytes require the exact immutable historical task-input universe that the provider operation answered.", {
    retryable: true, providerCallId: text(historical?.providerCallId) || null, executionId: text(historical?.executionId) || null,
    slotId: text(historical?.slotId) || null, task: text(historical?.task) || null,
  });
}
function historicalInputIntegrityInvalid(historical = {}, recordedDigest = null, observedDigest = null) {
  fail("MOVIE_MENTOR_PROVIDER_RECOVERY_INPUT_INTEGRITY_INVALID", "Historical provider reconstruction input no longer reproduces its immutable recorded digest.", {
    retryable: false, providerCallId: text(historical?.providerCallId) || null, executionId: text(historical?.executionId) || null,
    slotId: text(historical?.slotId) || null, task: text(historical?.task) || null,
    recordedDigest: text(recordedDigest) || null, observedDigest: text(observedDigest) || null,
  });
}
...
async function recoverPreviouslyAdmittedProviderResult({ decision = null, execution = null, slotId = null, task = null, input = null, recoverProviderOutcome = null, readProviderOperation = null, reconstructRecoveredResult = null } = {}) {
  ...
  const reconstructed = await reconstructRecoveredResult({
    input: clone(reconstructionAuthority.historicalInput), providerOperation,
    recoveredProviderResponse: bound.recoveredProviderResponse, recovery: freeze({ ...recovery }),
  });
