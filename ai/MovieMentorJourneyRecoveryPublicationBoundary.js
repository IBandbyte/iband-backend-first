...
const MOVIE_MENTOR_JOURNEY_RECOVERY_PUBLICATION_BOUNDARY_VERSION = "1.2.0";
function cleanString(value){return typeof value==="string"?value.trim():"";}
function validOwnershipRevision(value){return Number.isSafeInteger(value)&&value>=1;}
...
function createMovieMentorJourneyRecoveryPublicationBoundary({requestAuthority=createMovieMentorJourneyRecoveryRequestAuthority(),applyRecoveryTransition=applyMovieMentorJourneyRecoveryTransition,recoveryTransitionDeps=undefined,createPublicationWriteAuthority=createMovieMentorJourneyRecoveryPublicationWriteAuthority}={}){
 ...
 async function publish({request=null,projectId=null,expectedRecoveryRevision=null,envelope=null}={}){
  ...
  if(authorization?.authorized!==true||cleanString(authorization.projectId)!==pid||!cleanString(authorization.principalId)||!cleanString(authorization.ownershipRef)||!validOwnershipRevision(authorization.ownershipRevision))fail("MOVIE_MENTOR_JOURNEY_RECOVERY_PUBLICATION_AUTHORIZATION_INVALID","Journey recovery publication received invalid authorization provenance.");
