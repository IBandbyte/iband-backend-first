...
function closed(reason){return Object.freeze({mounted:false,reason,creatorBasePath:CREATOR_BASE_PATH,stripeWebhookPath:STRIPE_WEBHOOK_PATH});}

async function mountMovieMentorProductionCommercialHttpIngress({app,env=process.env,stripe=null}={}){
 if(!app||typeof app.use!=="function"||typeof app.post!=="function")return closed("express-app-required");
 ...
 // Stripe signature verification must receive the exact bytes. This route is mounted
 // before the application's general JSON parser and is not authenticated as a creator route.
 app.post(STRIPE_WEBHOOK_PATH,express.raw({type:"application/json",limit:"512kb"}),async(req,res)=>{
  try{
   const result=await ingress.authority.ingest({provider:"stripe",delivery:{rawBody:req.body,signature:req.get("Stripe-Signature")||""}});
   return res.status(200).json({received:true,status:result?.status||"commercial-evidence-processed"});
  }catch(error){
   const code=text(error?.code)||"MOVIE_MENTOR_COMMERCIAL_PROVIDER_INGRESS_FAILED";
   const clientFault=code.includes("SIGNATURE")||code.includes("DELIVERY_INVALID")||code.includes("PROVIDER_NOT_CONFIGURED");
   return res.status(clientFault?400:500).json({received:false,code});
  }
 });
 app.use(CREATOR_BASE_PATH,creator.router);
 return Object.freeze({mounted:true,reason:"commercial-http-ingress-mounted",creatorBasePath:CREATOR_BASE_PATH,stripeWebhookPath:STRIPE_WEBHOOK_PATH,version:VERSION});
}
