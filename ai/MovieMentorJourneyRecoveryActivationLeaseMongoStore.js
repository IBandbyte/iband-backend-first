...
function createMovieMentorJourneyRecoveryActivationLeaseMongoStore({ mongoModel = null, connect = ensureConnection, readPhysicalIndexes = null } = {}) {
  ...
  async function createLease(record = {}) { await physicalReadyBoundary(); const next = candidate(record); if (next.leaseGeneration !== 1) fail("MOVIE_MENTOR_RECOVERY_ACTIVATION_LEASE_MONGO_CREATE_GENERATION_INVALID", "First activation lease generation must be 1."); try { const written = normalize(await storeModel().create(next)); if (!sameIdentity(written, next) || written.leaseGeneration !== next.leaseGeneration || written.leaseReference !== next.leaseReference || written.fencingToken !== next.fencingToken || written.acquiredAt !== iso(next.acquiredAt) || written.expiresAt !== iso(next.expiresAt)) fail("MOVIE_MENTOR_RECOVERY_ACTIVATION_LEASE_MONGO_CREATE_RETURN_BINDING_INVALID", "Activation lease create returned a different durable holder or fencing universe.", { retryable: true }); return written; } catch (error) { if (error?.code === 11000) return null; throw error; } }
