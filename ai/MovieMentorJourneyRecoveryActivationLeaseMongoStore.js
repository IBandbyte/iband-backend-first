...
function createMovieMentorJourneyRecoveryActivationLeaseMongoStore({ mongoModel = null, connect = ensureConnection, readPhysicalIndexes = null } = {}) {
  ...
  async function replaceLease(record = {}, expected = {}) {
    ...
    const written = await storeModel().findOneAndUpdate(filter, { $set: next }, { new: true, runValidators: true }).lean().exec(); if (!written) return null; const normalizedWritten = normalize(written); if (!sameIdentity(normalizedWritten, next) || normalizedWritten.leaseGeneration !== next.leaseGeneration || normalizedWritten.leaseReference !== next.leaseReference || normalizedWritten.fencingToken !== next.fencingToken || normalizedWritten.acquiredAt !== iso(next.acquiredAt) || normalizedWritten.expiresAt !== iso(next.expiresAt)) fail("MOVIE_MENTOR_RECOVERY_ACTIVATION_LEASE_MONGO_RETURN_BINDING_INVALID", "Activation lease CAS returned a different durable holder or fencing universe.", { retryable: true }); return normalizedWritten;
  }
