...
function closed(reason, extras = {}) {
  return freeze({ ready: false, reason, activationAuthority: null, processInstanceId: "", deploymentId: text(extras.deploymentId), version: VERSION, domain: DOMAIN, bootWired: true, ...extras });
}
...
function createMovieMentorJourneyRecoveryProductionBootActivation({
  ...
  if (!composition || typeof composition.authorizeActivation !== "function") {
    fail("MOVIE_MENTOR_RECOVERY_PRODUCTION_BOOT_ACTIVATION_AUTHORITY_INVALID", "Certified composition did not expose activation authority.");
  }
  return freeze({ ready: true, reason: "certified-durable-composition-wired-for-boot", activationAuthority: composition.authorizeActivation.bind(composition), processInstanceId, deploymentId, version: VERSION, domain: DOMAIN, bootWired: true, compositionStatus });
}
...
function getMovieMentorJourneyRecoveryProductionBootActivationStatus({ env = process.env } = {}) {
  ...
  return freeze({ version: VERSION, domain: DOMAIN, ready: Boolean(deploymentId && compositionStatus?.ready && compositionStatus?.durable === true), deploymentConfigured: Boolean(deploymentId), compositionReady: Boolean(compositionStatus?.ready && compositionStatus?.durable === true), bootWired: true, deploymentEnv: DEPLOYMENT_ENV });
}
