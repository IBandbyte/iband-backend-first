...
function storeCapabilityProven(status){return status?.domain===STORE_DOMAIN&&status?.configured===true&&status?.atomicity===ATOMICITY&&status?.entitlementCollection===ENTITLEMENT_COLLECTION&&status?.issuanceCollection===ISSUANCE_COLLECTION&&status?.evidenceIdentityUnique===true&&status?.entitlementMutationAtomic===true&&status?.issuanceReceiptDurable===true&&status?.currentEntitlementRead===true&&status?.processLocalFallback===false;}
function createMovieMentorEntitlementIssuanceAuthority({store=null,allowedEvidenceSources=null,allowedEvidenceKinds=null}={}){
 if(typeof store?.issue!=="function"||typeof store?.resolveCurrentEntitlement!=="function")fail("MOVIE_MENTOR_ENTITLEMENT_ISSUANCE_AUTHORITY_REQUIRED","Entitlement issuance requires durable atomic issuance and current-entitlement read authority.");
 const storeStatus=ownedStatus(store);
 if(!storeCapabilityProven(storeStatus))fail("MOVIE_MENTOR_ENTITLEMENT_ISSUANCE_STORE_CAPABILITY_NOT_PROVEN","Entitlement issuance requires exact store-owned durable atomic issuance and current-entitlement capability proof.");
 ...
 async function resolveCurrentEntitlement({principalId}={}){const principal=text(principalId);if(!principal)fail("MOVIE_MENTOR_ENTITLEMENT_CURRENT_PRINCIPAL_REQUIRED","Current entitlement lookup requires principal identity.");const current=await store.resolveCurrentEntitlement({principalId:principal});if(!current)return null;if(text(current.principalId)!==principal||!["active","suspended"].includes(text(current.status))||!Number.isSafeInteger(current.entitlementRevision)||current.entitlementRevision<1)fail("MOVIE_MENTOR_ENTITLEMENT_CURRENT_RECORD_INVALID","Durable current entitlement does not bind requested principal and status.");return Object.freeze(current);}
 ...
 const status=Object.freeze({version:VERSION,domain:DOMAIN,durableAtomicIssuance:true,evidenceIdentityUnique:true,entitlementMutationAtomic:true,issuanceReceiptDurable:true,currentEntitlementRead:true,storeCapabilityProven:true,storeStatus,processLocalFallback:false});
 return Object.freeze({issueVerifiedEvidence,resolveCurrentEntitlement,getStatus:()=>status});
}
