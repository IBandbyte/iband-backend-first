...
function entitlementProven(authority,status){return isMovieMentorProductionEntitlementIssuanceAuthorityOwnedProof(authority,status)&&status?.domain===ENTITLEMENT_DOMAIN&&status?.production===true&&status?.durableAtomicIssuance===true&&status?.currentEntitlementRead===true&&status?.ownerBoundProof===true&&status?.processLocalFallback===false;}
function checkoutStatus(entitlementStatus){return Object.freeze({version:VERSION,domain:AUTHORITY_DOMAIN,production:true,durableCheckoutBinding:true,checkoutBindingResolution:true,providerPaymentReferenceBinding:true,providerPaymentReferenceResolution:true,openCheckoutRevocation:true,serverOwnedIdempotency:true,purchaseIntentProvenanceRequired:true,currentEntitlementProvenanceRequired:true,currentEntitlementOwnerProofRequired:true,currentEntitlementStatus:entitlementStatus,explicitProviderRequired:true,processLocalFallback:false});}
function createMovieMentorProductionCommercialCheckoutComposition({purchaseIntentAuthority,entitlementAuthority,providers={}}={}){
 ...
 if(typeof entitlementAuthority?.resolveCurrentEntitlement!=="function"||!entitlementProven(entitlementAuthority,entitlementStatus))fail("MOVIE_MENTOR_CHECKOUT_ENTITLEMENT_AUTHORITY_REQUIRED","Production checkout requires owner-proven current durable entitlement authority.");
 const storeStatus=getMovieMentorCommercialCheckoutBindingMongoStoreStatus();
 if(storeStatus?.configured!==true||storeStatus?.durable!==true||storeStatus?.providerPaymentReferenceBinding!==true||storeStatus?.providerPaymentReferenceResolution!==true||storeStatus?.unpaidCompletedCheckoutDiscovery!==true||storeStatus?.processLocalFallback!==false)fail("MOVIE_MENTOR_CHECKOUT_BINDING_STORE_NOT_CONFIGURED","Production checkout requires durable Mongo-backed checkout, provider-payment binding and open-checkout discovery authority.");
